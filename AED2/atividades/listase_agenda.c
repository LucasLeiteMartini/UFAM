...
void* acessar_com_comparacao(t_lse* lse, void* carga){
    if(lse->inicio){
        if(lse->comparar(lse->inicio->carga_util,carga) == 0){
            return lse->inicio->carga_util;
        }else{
            t_elemento_lse* cam = lse->inicio;
            while(cam->prox){
                if(lse->comparar(cam->prox->carga_util,carga) == 0){
...
void* remove_elem(t_lse* lse, void* chave){
    void* carga_util = NULL;
    t_elemento_lse* tmp = NULL;
    t_elemento_lse* cam = lse->inicio;
    if (lse->inicio && lse->comparar(lse->inicio->carga_util, chave) == 0){        
        carga_util = remover_lse(lse);
        return carga_util;
    }else{
        while((cam->prox)) {
            if (lse->comparar(cam->prox->carga_util, chave) == 0) {
                tmp = cam->prox;
                carga_util = tmp->carga_util;
                cam->prox = tmp->prox;
                free(tmp);
                return carga_util;
            }
